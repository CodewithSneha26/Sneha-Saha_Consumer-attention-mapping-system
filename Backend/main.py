...
@app.get("/trends-over-time")
def get_trends_over_time(db: Session = Depends(get_db)):
    ...
    # Group attention time by date
    attention_by_date = defaultdict(int)
    for r in attention_records:
        try:
            date_str = r.created_at[:10]  # YYYY-MM-DD
            attention_by_date[date_str] += r.duration_seconds
        except Exception:
            continue
    ...
    for i in interactions:
        try:
            date_str = i.created_at[:10]
            interactions_by_date[date_str] += 1
            if "Purchased" in i.interaction_type:
                purchases_by_date[date_str] += 1
        except Exception:
            continue

    all_dates = sorted(set(list(attention_by_date.keys()) + list(interactions_by_date.keys())))

    trend_data = []
    for date in all_dates:
        trend_data.append({
            "date": date,
            "total_attention_seconds": attention_by_date.get(date, 0),
            "total_interactions": interactions_by_date.get(date, 0),
            "total_purchases": purchases_by_date.get(date, 0)
        })

    return {
        "trend_data": trend_data,
        "has_multiple_sessions": len(all_dates) > 1
    }